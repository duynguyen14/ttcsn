    ...
    return ( 
       <div>
        ...
        <div className='flex justify-between items-center px-6 py-3 lg:px-16 xl:px-32 relative'>
            ...
            {/*logo */}
            <div className=''>
                <Link to="/" className='text-primary uppercase text-xl font-bold font-Montserrat'>
                    PhdTech
                </Link>
            </div>
            {/*search input */}
            ...
            interactive={true}
            render={attrs => (
                <div className="box bg-white lg:ml-[100px] xl:ml-[200px]" tabIndex="-1" {...attrs}>
            ...
            {/*other button */}
                    <div className='hidden lg:flex justify-between items-center gap-x-5 xl:gap-x-10'>
                        <li className='flex justify-between items-center gap-x-2'>
                            <FaPhone className='text-yellow-600'/> <p className='text-primary font-bold cursor-pointer hover:text-yellow-600'>0123456789</p>
                        </li>
                        <li className='flex justify-between items-center gap-x-2'>
                            <IoTimeSharp className='text-yellow-600'/> <p className='text-primary font-bold cursor-pointer hover:text-yellow-600'>8:30-18:30</p>
                        </li>
                        <li className='list-none text-xl font-bold hover:text-primary relative group text-center'>
                            <Link to={UserStatus ? "/profile": "/login"}>
                                <FaUser className="w-[100px] text-center"/>
                                <div className="absolute text-xs whitespace-nowrap hidden group-hover:block cursor-pointer text-center w-[100px] py-1">
                                    {
                                        UserStatus
                                        ? <p>
                                          {Userinfor.name}
                        ...
                        {UserStatus?<div></div>:
                            <li className="list-none text-[10px] font-bold ">
                                <Link to={"/regester"} className="px-2 py-2 bg-primary hover:bg-primary/70 rounded-md text-white whitespace-nowrap">
                                    Đăng Ký
                                </Link>
                            </li>
                        }
                        <li className='list-none text-xl font-bold hover:text-primary relative group text-center'>
                            ...
                            <p className="absolute text-xs whitespace-nowrap hidden group-hover:block cursor-pointer text-center w-full py-1">Giỏ hàng</p>
                            </Link>
                        </li>
                    </div>
            </div>
        ...
        {
                    <div className={`absolute block justify-around items-start w-full 
                    transition-all transform duration-500 ease-in-out
                    h-auto bg-gray-50 top-[45px] ${ismenu? " opacity-100  translate-x-0" : " opacity-0 translate-x-full"} lg:hidden z-10`}
                    >
                        <ul className="font-Montserrat font-semibold py-2 text-xs text-red-500 hover:bg-gray-50 w-full text-center uppercase border-b-slate-200 md:text-xl">
                            <p className="decoration-red-500">Sản phẩm</p>
                            {
                                titles_1.map((title_1,index)=>{
                                    return (
                                        <li key={index} className=" font-Montserrat font-semibold text-center uppercase text-gray-500 hover:text-white hover:bg-primary/50 py-2 transition-all duration-500 ease-in-out md:text-lg md:py-5"
                                        onClick={()=>setIsmenu(!ismenu)}>
                                            <Link to={title_1.link}>
                                            {title_1.title}
                                            </Link>
